...
header('location:02userLogin.php');
    ...
    if(isset($_POST['submit'])){
        $query=mysqli_query($con,"SELECT password from users where password='".md5($_POST['password'])."' && idNumber='".$_SESSION['login']."' ");
        $num=mysqli_fetch_array($query);

        if($num>0){
            $updateQuery=mysqli_query($con,"update users set password='".md5($_POST['newpassword'])."' where idNumber='".$_SESSION['login']."' ");
